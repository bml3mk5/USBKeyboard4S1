...
/* static in case exception condition would stop auto variable being created */
static unsigned long _excep_code;
...
#ifdef DEBUG_ENABLE
    UART_Flush();
    UART_PutStringDirect("Exception\r\n");
    UART_PutHexaDirect((uint8_t *)&_excep_code, sizeof(_excep_code));
    UART_PutHexaDirect((uint8_t *)&_excep_addr, sizeof(_excep_addr));
#endif
